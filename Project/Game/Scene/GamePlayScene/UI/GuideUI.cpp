...
void GuideUI::Initialize() {
	...
	mouseSprite->Initialize(kFolderPath_ + "Mouse.png");
	...
	buttonASprite->Initialize(kFolderPath_ + "ButtonA.png");
	...
	buttonDSprite->Initialize(kFolderPath_ + "ButtonD.png");
	...
	buttonESCSprite->Initialize(kFolderPath_ + "Pause.png");
...
void GuideUI::Update() {
	...
	if (input->PushMouseButton(MouseButton::Left)) {
		...
		if (isShooting == false) {
			mouseSprite->SetTexture(kFolderPath_ + "MouseClick.png"); // 射撃時の画像(黄色)へ
	...
	// 押していない時
		...
		// 「射撃フラグが立っていた」時だけ元に戻す
		if (isShooting == true) {
			mouseSprite->SetTexture(kFolderPath_ + "Mouse.png"); // 通常時の画像(水色)へ
	...
	if (input->PushKey('A')) {
		...
		if (isRollLeft == false) {
			buttonASprite->SetTexture(kFolderPath_ + "PushA.png"); // 押下時の画像(黄色)へ
	...
	// 押していない時
		...
		if (isRollLeft == true) {
			buttonASprite->SetTexture(kFolderPath_ + "ButtonA.png"); // 通常時の画像(水色)へ
	...
	if (input->PushKey('D')) {
		...
		if (isRollRight == false) {
			buttonDSprite->SetTexture(kFolderPath_ + "PushD.png"); // 押下時の画像(黄色)へ
	...
	// 押していない時
		...
		if (isRollRight == true) {
			buttonDSprite->SetTexture(kFolderPath_ + "ButtonD.png"); // 通常時の画像(水色)へ
	...
	//	if (isPause == false) {
	//		buttonESCSprite->SetTexture(kFolderPath_ + "Back.png"); // 戻るの画像へ
	//		isPause = true; // 状態を「ポーズ中」にする
	//	}
	//}
	//else {
	//	if (isPause == true) {
	//		buttonESCSprite->SetTexture(kFolderPath_ + "Pause.png"); // ポーズの画像へ
