...
void RadialBlurFilter::ShowImGui() {
	...
	if (ImGui::TreeNode("RadialBlurFilter")) {
		...
		ImGui::ColorEdit3("BlurColor", &configData_->glowColor.x);
		ImGui::SliderFloat("BlurStrength", &configData_->blurStrength, 0.0f, 1.0f, "%.3f");
		ImGui::SliderFloat2("Center", &configData_->center.x, 0.0f, 1.0f, "%.2f");
...
void RadialBlurFilter::CreateConfigData() {
	...
	// 初期値をセット
	configData_->glowColor = { 1.0f, 1.0f, 1.0f }; // 光の色
	configData_->blurStrength = 0.01f;		 // ぼかし強度
	configData_->center = { 0.5f, 0.5f }; // 画面中央
}
